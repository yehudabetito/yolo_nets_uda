...
def yolo_main(chosen_image):    
    ...
    #yolo_model.summary()  ########################### this is the summary!!
    ...
    boxes, scores, classes = yolo_eval(yolo_outputs[:4], image_shape)
    ...
    #imshow(output_image)
    #lay_num = sess.run( yolo_model.layers[72].output,feed_dict={yolo_model.input:image_data})
    
    return out_boxes, out_classes,input_image.size,len(class_names)
